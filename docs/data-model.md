...
Personal FiAt streaks are derived from the owner's reflection records, including private records, with one streak day per distinct `fiatDateKey`. The authenticated `/api/fiat/leaderboard` endpoint derives numeric activity using Firebase Admin, counts every FiAt entry without a daily cap, joins only public `socialProfiles`, and returns no reflection content or privacy metadata. Firestore client rules remain unchanged for cross-user reflection access; private reflections never become directly readable for leaderboard purposes.
