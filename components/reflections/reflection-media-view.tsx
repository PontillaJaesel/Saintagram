...
export function ReflectionMediaView({ media, compact = false }: { media?: ReflectionMedia[]; compact?: boolean }) {
  ...
  const preview = (
    <div className={`relative mt-4 overflow-hidden rounded-2xl bg-sage-50 ${compact ? "max-w-sm" : "max-w-lg"}`}>
      <div
        ref={sliderRef}
      ...
      >
        {items.map((item, index) => (
          <button
            key={item.path}
            type="button"
            className="relative aspect-[4/5] w-full shrink-0 snap-start overflow-hidden bg-black/5"
            onClick={() => { setDownloadError(""); setFullscreen(index); }}
            aria-label={`Open ${item.type} ${index + 1} in full screen`}
          >
            {item.type === "video" ? (
              <><video src={item.url} className="size-full object-contain" playsInline muted preload="metadata" /><span className="absolute inset-0 grid place-items-center"><span className="grid size-14 place-items-center rounded-full bg-black/60 text-white"><Play className="ml-1 size-7" fill="currentColor" /></span></span></>
            ) : (
              <img src={item.url} alt={`Reflection photo ${index + 1}`} className="size-full object-contain" loading="lazy" />
            )}
      ...
      {items.length > 1 && <>
        {active > 0 && <button type="button" className="absolute left-2 top-1/2 grid size-10 -translate-y-1/2 place-items-center rounded-full bg-black/55 text-white" onClick={() => moveTo(active - 1)} aria-label="Previous photo"><ChevronLeft /></button>}
        {active < items.length - 1 && <button type="button" className="absolute right-2 top-1/2 grid size-10 -translate-y-1/2 place-items-center rounded-full bg-black/55 text-white" onClick={() => moveTo(active + 1)} aria-label="Next photo"><ChevronRight /></button>}
        <span className="absolute bottom-3 left-1/2 -translate-x-1/2 rounded-full bg-black/60 px-3 py-1 text-xs font-bold text-white">{active + 1} / {items.length}</span>
      </>}
    </div>
  );
