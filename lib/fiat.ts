...
export function eligibleFiatCount(posts: readonly ReflectionPost[],start:string,end:string):number{return posts.filter(post=>{if(!post.fiatCategory)return false;const key=reflectionFiatDateKey(post);return key>=start&&key<=end;}).length;}
