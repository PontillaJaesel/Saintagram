...
/**
 * Reads the duration of a local video without uploading it.
 *
 * This is used both:
 * - when the user initially chooses a video
 * - immediately before Firebase upload validation
 */
...
/**
 * Final validation before any reflection media
 * is uploaded to Firebase Storage.
 *
 * Even if somebody somehow bypasses the picker,
 * a video longer than 60 seconds will still fail here.
 */
export async function validateReflectionMedia(
  files: File[]
): Promise<void> {
  if (!files.length) {
    return;
  ...
  if (videos.length) {
    ...
    // Small tolerance accounts for differences
    // in encoded frame timing.
    if (
      duration >
      LIMITS.reflectionVideoSeconds + 0.1
    ) {
      throw new Error(
        `The video must be ${LIMITS.reflectionVideoSeconds} seconds or shorter.`
      );
    }
  }
}

export function reflectionMediaId(): string {
  return crypto.randomUUID();
}

export async function uploadReflectionMedia(
  userId: string,
  reflectionId: string,
  files: File[],
  isPrivate = false
): Promise<ReflectionMedia[]> {
  // Keep this validation here.
  ...
  try {
    for (const file of files) {
      ...
      const path =
        ...
        `${crypto.randomUUID()}.${extension(file)}`;
    ...
    throw error instanceof Error
      ? error
      : new Error(
          "Reflection media could not be uploaded."
        );
  }
}

export async function deleteReflectionMedia(
...
export async function reflectionMediaUrl(
  path: string
): Promise<string> {
  const services =
    getFirebaseServices();
...
export async function deleteAllReflectionMedia(
  userId: string
): Promise<void> {
  const services =
    getFirebaseServices();
